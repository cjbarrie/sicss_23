...
library(quanteda) #for some conText pre-processing
```
...
tweets_corpus <- corpus(text, text_field = "tweet", docvars = "party_value")
...
feats <- dfm(toks, verbose = TRUE) %>% 
  dfm_trim(min_termfreq = COUNT_MIN) %>% featnames()

# leave the pads so that non-adjacent words will not become adjacent
toks_feats <- tokens_select(toks, feats, padding = TRUE)

toks_fcm <- fcm(toks_feats, context = "window", window = WINDOW_SIZE, 
...
url <- "https://github.com/cjbarrie/sicss_23/blob/main/data/wordembed/local_glove.rds?raw=true"
...
selected_words <- df_glove_umap %>% inner_join(y=select, by= "word") 
...
3.  Output nearest synonyms for target words in a formatted table using the `kableExtra` package.
4.  Investigate the [conText](https://github.com/prodriguezsosa/conText/) package and implement an analysis with e.g., the `nns_ratio()` function or another function. See [this guide](https://cran.r-project.org/web/packages/conText/vignettes/quickstart.html) for some example analyses. (Tip: see [here](https://github.com/prodriguezsosa/conText/blob/master/vignettes/quickstart.md#estimating-the-transformation-matrix) for how to estimate the transformation matrix).
